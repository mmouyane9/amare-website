...
(function () {
  ...
  var PAGE_SLUG = '/Who%20are%20we/our-values.html';
  ...
  /* ------------------------------------------------------------------
     CORE VALUES INTRO — eyebrow, heading, lead
     ------------------------------------------------------------------ */
  function injectCoreIntro(d) {
    var eyebrow = el('.ov-intro-head .eyebrow');
    if (eyebrow && d.eyebrow) eyebrow.textContent = d.eyebrow;

    var title = el('.ov-intro-title');
    if (title && d.heading) title.textContent = d.heading;

    var lead = el('.ov-intro-lead');
    if (lead && d.description) lead.textContent = d.description;
  }

  /* ------------------------------------------------------------------
     VALUES GRID — section-head + 8 value cards
     ------------------------------------------------------------------ */
  function injectValuesGrid(d) {
    var eyebrow = el('#ovValues .section-head .eyebrow');
    if (eyebrow && d.eyebrow) eyebrow.textContent = d.eyebrow;

    var title = el('#ovValues .section-title');
    if (title && d.heading) title.textContent = d.heading;

    var desc = el('#ovValues .section-desc');
    if (desc && d.description) desc.textContent = d.description;

    var cards = els('.ov-value-card');
    if (d.cards) {
  ...
  /* ------------------------------------------------------------------
     HOW WE APPLY — section-head, image, floats, timeline steps
     ------------------------------------------------------------------ */
  function injectApply(d) {
    var eyebrow = el('#ovApply .section-head .eyebrow');
    if (eyebrow && d.eyebrow) eyebrow.textContent = d.eyebrow;

    var title = el('#ovApply .section-title');
    if (title && d.heading) title.textContent = d.heading;

    var desc = el('#ovApply .section-desc');
    if (desc && d.description) desc.textContent = d.description;

    var img = el('.ov-apply-frame img');
    if (img && d.image) {
      img.src = d.image.url || '';
      img.alt = d.image.alt || '';
    }

    var floats = els('.ov-apply-float');
    if (d.highlights) {
    ...
    var steps = els('.ov-apply-step');
    if (d.steps) {
      for (var s = 0; s < Math.min(steps.length, d.steps.length); s++) {
        var h3 = steps[s].querySelector('h3');
        var p = steps[s].querySelector('p');
        if (h3) h3.textContent = d.steps[s].heading || '';
        if (p) p.textContent = d.steps[s].description || '';
      }
    }
  }

  /* ------------------------------------------------------------------
     STATISTICS — eyebrow, heading, stats
     ------------------------------------------------------------------ */
  function injectStatistics(d) {
    var eyebrow = el('#ovStats .section-head .eyebrow');
    if (eyebrow && d.description) eyebrow.textContent = d.description;

    var title = el('#ovStats .section-title');
    if (title && d.heading) title.textContent = d.heading;

    var stats = els('.ov-stat');
    if (d.stats) {
      for (var i = 0; i < Math.min(stats.length, d.stats.length); i++) {
        var num = stats[i].querySelector('.ov-stat-num');
        var label = stats[i].querySelector('.ov-stat-label');
        if (num) {
  ...
  function injectQuote(d) {
    var blockquote = el('.ov-quote-box blockquote');
    if (blockquote && d.quote) blockquote.textContent = '«' + d.quote + '»';

    var figcaption = el('.ov-quote-box figcaption');
    if (figcaption && d.attribution) {
      var rule = figcaption.querySelector('.ov-quote-rule');
  ...
  function dispatchSection(section) {
    ...
    switch (type) {
      ...
      case 'statistics': console.log('[Values CMS] Rendering statistics'); return injectStatistics(data);
      case 'cta':        console.log('[Values CMS] Rendering cta');        return injectCta(data);
      case 'custom': {
  ...
  function renderAll() {
    if (_lastSections && _lastSections.length > 0) {
      console.log('[Values CMS] Re-rendering', _lastSections.length, 'CMS sections...');
      CUSTOM_ORDER = 0;
  ...
  function init() {
    loadSections(function (sections) {
      ...
      console.log('[Values CMS] Rendering complete —', sections.length, 'sections rendered');
    });
