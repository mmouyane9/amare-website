-- ============================================================================
-- AMARE Our Mission — Non-Destructive CMS Seed
...
BEGIN
    ...
    SELECT gen_random_uuid(), target_id, 'hero', 'ms.hero', 'رسالتنا', 'القسم الرئيسي لصفحة الرسالة',
        $CONTENT${
        ...
        '{}'::jsonb, '{}'::jsonb, true, 1
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'ms.hero');

    -- =====================================================================
    -- 2. MISSION STATEMENT  (section_key: ms.mission)
    -- =====================================================================
    INSERT INTO page_sections (id, page_id, section_type, section_key, title, description, content, settings, styles, visible, sort_order)
    SELECT gen_random_uuid(), target_id, 'custom', 'ms.mission', 'رسالتنا', 'بيان الرسالة والغاية',
        $CONTENT${
          "eyebrow": "غايتنا",
          "heading": "رسالتنا",
        ...
        '{}'::jsonb, '{}'::jsonb, true, 2
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'ms.mission');

    -- =====================================================================
    -- 3. PILLARS  (section_key: ms.pillars)
    -- =====================================================================
    INSERT INTO page_sections (id, page_id, section_type, section_key, title, description, content, settings, styles, visible, sort_order)
    SELECT gen_random_uuid(), target_id, 'custom', 'ms.pillars', 'نعمل من أجل', 'محاور العمل الأربعة',
        $CONTENT${
          "eyebrow": "محاور عملنا",
          "heading": "نعمل من أجل",
        ...
        '{}'::jsonb, '{}'::jsonb, true, 3
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'ms.pillars');

    -- =====================================================================
    -- 4. HOW WE ACHIEVE  (section_key: ms.approach)
    -- =====================================================================
    INSERT INTO page_sections (id, page_id, section_type, section_key, title, description, content, settings, styles, visible, sort_order)
    SELECT gen_random_uuid(), target_id, 'custom', 'ms.approach', 'كيف نحقق رسالتنا؟', 'منهجية العمل والبرامج',
        $CONTENT${
        ...
        '{}'::jsonb, '{}'::jsonb, true, 4
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'ms.approach');

    -- =====================================================================
    -- 5. QUOTE  (section_key: ms.quote)
    -- =====================================================================
    INSERT INTO page_sections (id, page_id, section_type, section_key, title, description, content, settings, styles, visible, sort_order)
    SELECT gen_random_uuid(), target_id, 'custom', 'ms.quote', 'اقتباس ملهم', 'رسالة ملهمة من الجمعية',
        $CONTENT${
        ...
        '{}'::jsonb, '{}'::jsonb, true, 5
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'ms.quote');

    -- =====================================================================
    -- 6. CTA  (section_key: ms.cta)
    -- =====================================================================
    INSERT INTO page_sections (id, page_id, section_type, section_key, title, description, content, settings, styles, visible, sort_order)
    SELECT gen_random_uuid(), target_id, 'cta', 'ms.cta', 'ساهم معنا في تحقيق رسالتنا', 'دعوة للانخراط والمشاركة',
        $CONTENT${
        ...
        '{}'::jsonb, '{}'::jsonb, true, 6
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'ms.cta');

    -- =====================================================================
    -- 7. FOOTER  (section_key: ms.footer)
    -- =====================================================================
    INSERT INTO page_sections (id, page_id, section_type, section_key, title, description, content, settings, styles, visible, sort_order)
    SELECT gen_random_uuid(), target_id, 'custom', 'ms.footer', 'Footer', 'تذييل الصفحة',
        $CONTENT${
          "_renderer": "footer",
        ...
        '{}'::jsonb, '{}'::jsonb, true, 7
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'ms.footer');
