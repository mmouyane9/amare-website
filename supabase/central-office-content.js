/* ==========================================================================
   Central Office — Content Service
   Loads page sections from Supabase and injects into the DOM.
   Slug: /Who%20are%20we/central-office.html
   ========================================================================== */

(function () {
  'use strict';

  var PAGE_SLUG = '/Who%20are%20we/central-office.html';
  ...
  function injectAbout(d) {
    var eyebrow = el('.co-about-inner .eyebrow');
    if (eyebrow && d.eyebrow) eyebrow.textContent = d.eyebrow;
    var title = el('.co-about-title');
    if (title && d.heading) title.textContent = d.heading;
    var lead = el('.co-about-lead');
    if (lead && d.description) lead.textContent = d.description;
  }

  /* ------------------------------------------------------------------
     TASKS — section-head + 6 task cards
     ------------------------------------------------------------------ */
  function injectTasks(d) {
    var eyebrow = el('#coTasks .section-head .eyebrow');
    if (eyebrow && d.eyebrow) eyebrow.textContent = d.eyebrow;
    var title = el('#coTasks .section-title');
    if (title && d.heading) title.textContent = d.heading;
    var desc = el('#coTasks .section-desc');
    if (desc && d.description) desc.textContent = d.description;

    var cards = els('.co-task-card');
    if (d.cards) {
  ...
  /* ------------------------------------------------------------------
     WORKING PROCESS — section-head + 4 process steps
     ------------------------------------------------------------------ */
  function injectProcess(d) {
    var eyebrow = el('#coProcess .section-head .eyebrow');
    if (eyebrow && d.eyebrow) eyebrow.textContent = d.eyebrow;
    var title = el('#coProcess .section-title');
    if (title && d.heading) title.textContent = d.heading;
    var desc = el('#coProcess .section-desc');
    if (desc && d.description) desc.textContent = d.description;

    var steps = els('.co-process-step');
    if (d.steps) {
      for (var i = 0; i < Math.min(steps.length, d.steps.length); i++) {
        var num = steps[i].querySelector('.co-process-num');
  ...
  function injectQuote(d) {
    var blockquote = el('.co-quote-inner blockquote');
    if (blockquote && d.quote) blockquote.textContent = '«' + d.quote + '»';
    var figcaption = el('.co-quote-inner figcaption');
    if (figcaption && d.attribution) {
      var rule = figcaption.querySelector('.co-quote-rule');
  ...
  function loadSections(callback) {
    ...
    if (!S || !S.getClient) { console.log('[CentralOffice CMS] Supabase not available'); return callback(null); }
    var client = S.getClient();
    if (!client) { console.log('[CentralOffice CMS] No Supabase client'); return callback(null); }

    console.log('[CentralOffice CMS] Loading page — slug:', PAGE_SLUG);
    ...
    client.from('pages')
      ...
      .then(function (pageResult) {
        if (pageResult.error || !pageResult.data) {
          console.log('[CentralOffice CMS] Page not found or not published:', pageResult.error);
          return callback(null);
        }
        var pageId = pageResult.data.id;
        console.log('[CentralOffice CMS] Page UUID:', pageId);
        ...
        client.from('page_sections')
          ...
          .then(function (secResult) {
            if (secResult.error || !secResult.data) {
              console.log('[CentralOffice CMS] No sections found:', secResult.error);
              return callback(null);
            }
            var rows = secResult.data;
            console.log('[CentralOffice CMS] Loaded page_sections count:', rows.length);
            ...
            for (var i = 0; i < rows.length; i++) {
              ...
              console.log('[CentralOffice CMS] Section ' + i + ': type=' + row.section_type + ', renderer=' + (data._renderer || 'none'));
          ...
          .catch(function (err) { console.log('[CentralOffice CMS] Error:', err); return callback(null); });
      })
      .catch(function (err) { console.log('[CentralOffice CMS] Error:', err); return callback(null); });
  }
  ...
  function dispatchSection(section) {
    ...
    switch (type) {
      case 'hero': console.log('[CentralOffice CMS] Rendering hero'); return injectHero(data);
      case 'cta':  console.log('[CentralOffice CMS] Rendering cta');  return injectCta(data);
      case 'custom': {
        var renderer = data._renderer || '';
        if (renderer === 'footer') { console.log('[CentralOffice CMS] Rendering footer'); return injectFooter(data); }
        /* Position: 1→About, 2→Tasks, 3→Process, 4→Quote */
        var ci = 0; for (var i = 0; i < index; i++) ci++;
        if (ci === 0) { console.log('[CentralOffice CMS] Rendering about'); return injectAbout(data); }
        if (ci === 1) { console.log('[CentralOffice CMS] Rendering tasks'); return injectTasks(data); }
        if (ci === 2) { console.log('[CentralOffice CMS] Rendering process'); return injectProcess(data); }
        if (ci === 3) { console.log('[CentralOffice CMS] Rendering quote'); return injectQuote(data); }
        break;
      }
    }
  }
