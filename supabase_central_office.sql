-- ============================================================================
-- AMARE Central Office — Non-Destructive CMS Seed
...
BEGIN
    ...
    SELECT gen_random_uuid(), target_id, 'hero', 'co.hero', 'المكتب المركزي', 'القسم الرئيسي لصفحة المكتب المركزي',
        $CONTENT${
        ...
        '{}'::jsonb, '{}'::jsonb, true, 1
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'co.hero');

    -- =====================================================================
    -- 2. ABOUT THE CENTRAL OFFICE  (section_key: co.about)
    -- =====================================================================
    INSERT INTO page_sections (id, page_id, section_type, section_key, title, description, content, settings, styles, visible, sort_order)
    SELECT gen_random_uuid(), target_id, 'custom', 'co.about', 'نبذة عن المكتب المركزي', 'مقدمة عن المكتب المركزي للجمعية',
        $CONTENT${
        ...
        '{}'::jsonb, '{}'::jsonb, true, 2
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'co.about');

    -- =====================================================================
    -- 3. TASKS (6 CARDS)  (section_key: co.tasks)
    -- =====================================================================
    INSERT INTO page_sections (id, page_id, section_type, section_key, title, description, content, settings, styles, visible, sort_order)
    SELECT gen_random_uuid(), target_id, 'custom', 'co.tasks', 'مهام المكتب المركزي', 'المسؤوليات والاختصاصات',
        $CONTENT${
        ...
        '{}'::jsonb, '{}'::jsonb, true, 3
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'co.tasks');

    -- =====================================================================
    -- 4. WORKING PROCESS (TIMELINE)  (section_key: co.process)
    -- =====================================================================
    INSERT INTO page_sections (id, page_id, section_type, section_key, title, description, content, settings, styles, visible, sort_order)
    SELECT gen_random_uuid(), target_id, 'custom', 'co.process', 'آلية العمل', 'منهجية عمل المكتب المركزي',
        $CONTENT${
          "eyebrow": "دورة العمل",
          "heading": "آلية العمل",
        ...
        '{}'::jsonb, '{}'::jsonb, true, 4
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'co.process');

    -- =====================================================================
    -- 5. QUOTE  (section_key: co.quote)
    -- =====================================================================
    INSERT INTO page_sections (id, page_id, section_type, section_key, title, description, content, settings, styles, visible, sort_order)
    SELECT gen_random_uuid(), target_id, 'custom', 'co.quote', 'اقتباس ملهم', 'رسالة قيادية',
        $CONTENT${
        ...
        '{}'::jsonb, '{}'::jsonb, true, 5
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'co.quote');

    -- =====================================================================
    -- 6. CTA  (section_key: co.cta)
    -- =====================================================================
    INSERT INTO page_sections (id, page_id, section_type, section_key, title, description, content, settings, styles, visible, sort_order)
    SELECT gen_random_uuid(), target_id, 'cta', 'co.cta', 'ساهم معنا في بناء مستقبل أفضل', 'دعوة للانخراط والمشاركة',
        $CONTENT${
        ...
        '{}'::jsonb, '{}'::jsonb, true, 6
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'co.cta');

    -- =====================================================================
    -- 7. FOOTER  (section_key: co.footer)
    -- =====================================================================
    INSERT INTO page_sections (id, page_id, section_type, section_key, title, description, content, settings, styles, visible, sort_order)
    SELECT gen_random_uuid(), target_id, 'custom', 'co.footer', 'Footer', 'تذييل الصفحة',
        $CONTENT${
          "_renderer": "footer",
        ...
        '{}'::jsonb, '{}'::jsonb, true, 7
    WHERE NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = target_id AND section_key = 'co.footer');
