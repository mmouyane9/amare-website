-- ============================================================================
-- ANCPP (الشريك) — Full 8-section CMS seed
-- Page slug '/partners/ancpp' — the public page at /Our partners/ancpp.html
...
-- AssociationDetectionCentre. ONLY touches this page.
-- Content extracted verbatim from /Our partners/ancpp.html (real content).
-- ============================================================================
...
DECLARE
  v_pid UUID;
BEGIN
  SELECT id INTO v_pid FROM pages WHERE slug = '/partners/ancpp' LIMIT 1;
  IF v_pid IS NULL THEN
    INSERT INTO pages (title, slug, status) VALUES ('ANCPP', '/partners/ancpp', 'published') RETURNING id INTO v_pid;
  END IF;
  RAISE NOTICE 'page_id: %', v_pid;

  -- 1. HERO
  UPDATE page_sections SET section_type = 'hero',
    content = '{"heading":"ANCPP","subheading":"شريك وطني","description":"شريك وطني للجمعية المغربية لهواة البحث والاستكشاف، الجمعية الوطنية لمحترفي الصيد البحري بالمغرب.","backgroundImage":"","buttons":[{"id":"btn-pr-website","label":"زيارة الموقع الإلكتروني","url":"https://www.ancpp.ma","variant":"primary"},{"id":"btn-pr-contact","label":"تواصل مع الشريك","url":"#prForm","variant":"secondary"}]}'::jsonb,
    settings='{}'::jsonb, styles='{}'::jsonb
    WHERE page_id = v_pid AND sort_order = 1;
  IF NOT FOUND THEN INSERT INTO page_sections (id, page_id, section_type, visible, sort_order, content, settings, styles) VALUES (gen_random_uuid(), v_pid, 'hero', TRUE, 1, '{"heading":"ANCPP","subheading":"شريك وطني","description":"شريك وطني للجمعية المغربية لهواة البحث والاستكشاف، الجمعية الوطنية لمحترفي الصيد البحري بالمغرب.","backgroundImage":"","buttons":[{"id":"btn-pr-website","label":"زيارة الموقع الإلكتروني","url":"https://www.ancpp.ma","variant":"primary"},{"id":"btn-pr-contact","label":"تواصل مع الشريك","url":"#prForm","variant":"secondary"}]}'::jsonb, '{}'::jsonb, '{}'::jsonb); END IF;

  -- 2. ABOUT
  UPDATE page_sections SET section_type = 'custom',
    content = '{"_renderer":"partnerAbout","eyebrow":"عن ANCPP","heading":"شريككم الموثوق في مجال التنقيب والاستكشاف","image":"","paragraphs":["ANCPP هي الجمعية الوطنية لمحترفي الصيد البحري بالمغرب. تمثل الجمعية مصالح الصيادين المحترفين وتعمل على تطوير قطاع الصيد البحري وحماية الثروة السمكية الوطنية.","نعمل مع الجمعية المغربية لهواة البحث والاستكشاف في إطار شراكة وطنية تهدف إلى تعزيز التعاون في مجال الاستكشاف البحري وحماية البيئة البحرية المغربية."]}'::jsonb,
    settings='{}'::jsonb, styles='{}'::jsonb
    WHERE page_id = v_pid AND sort_order = 2;
  IF NOT FOUND THEN INSERT INTO page_sections (id, page_id, section_type, visible, sort_order, content, settings, styles) VALUES (gen_random_uuid(), v_pid, 'custom', TRUE, 2, '{"_renderer":"partnerAbout","eyebrow":"عن ANCPP","heading":"شريككم الموثوق في مجال التنقيب والاستكشاف","image":"","paragraphs":["ANCPP هي الجمعية الوطنية لمحترفي الصيد البحري بالمغرب. تمثل الجمعية مصالح الصيادين المحترفين وتعمل على تطوير قطاع الصيد البحري وحماية الثروة السمكية الوطنية.","نعمل مع الجمعية المغربية لهواة البحث والاستكشاف في إطار شراكة وطنية تهدف إلى تعزيز التعاون في مجال الاستكشاف البحري وحماية البيئة البحرية المغربية."]}'::jsonb, '{}'::jsonb, '{}'::jsonb); END IF;
  ...
  -- 6. CONTACT
  UPDATE page_sections SET section_type = 'custom',
    content = '{"_renderer":"partnerContact","heading":"تواصل معنا","description":"نحن هنا للإجابة على استفساراتكم.","email":"contact@ancpp.ma","phone":"+212 528 000 000","website":"www.ancpp.ma","address":"ميناء أكادير، أكادير، المغرب"}'::jsonb,
    settings='{}'::jsonb, styles='{}'::jsonb
    WHERE page_id = v_pid AND sort_order = 6;
  IF NOT FOUND THEN INSERT INTO page_sections (id, page_id, section_type, visible, sort_order, content, settings, styles) VALUES (gen_random_uuid(), v_pid, 'custom', TRUE, 6, '{"_renderer":"partnerContact","heading":"تواصل معنا","description":"نحن هنا للإجابة على استفساراتكم.","email":"contact@ancpp.ma","phone":"+212 528 000 000","website":"www.ancpp.ma","address":"ميناء أكادير، أكادير، المغرب"}'::jsonb, '{}'::jsonb, '{}'::jsonb); END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM page_sections WHERE page_id = v_pid AND sort_order = 7) THEN
    ...
    VALUES (gen_random_uuid(), v_pid, 'custom', NULL, TRUE, 7,
      '{"_renderer":"partnerForm","heading":"تواصل مع ANCPP","description":"أرسل لنا استفسارك وسنرد عليك في أقرب وقت."}'::jsonb, '{}'::jsonb, '{}'::jsonb);
  END IF;

  -- 8. CTA
  UPDATE page_sections SET section_type = 'custom',
    content = '{"_renderer":"partnerCta","heading":"هل أنت مهتم بالعمل مع هذا الشريك؟","description":"","buttons":[{"id":"btn-pr-cta-0","label":"زيارة الموقع الإلكتروني","url":"https://www.ancpp.ma","variant":"primary"},{"id":"btn-pr-cta-1","label":"تواصل مع الشريك","url":"#prForm","variant":"secondary"}]}'::jsonb,
    settings='{}'::jsonb, styles='{}'::jsonb
    WHERE page_id = v_pid AND sort_order = 8;
  IF NOT FOUND THEN INSERT INTO page_sections (id, page_id, section_type, visible, sort_order, content, settings, styles) VALUES (gen_random_uuid(), v_pid, 'custom', TRUE, 8, '{"_renderer":"partnerCta","heading":"هل أنت مهتم بالعمل مع هذا الشريك؟","description":"","buttons":[{"id":"btn-pr-cta-0","label":"زيارة الموقع الإلكتروني","url":"https://www.ancpp.ma","variant":"primary"},{"id":"btn-pr-cta-1","label":"تواصل مع الشريك","url":"#prForm","variant":"secondary"}]}'::jsonb, '{}'::jsonb, '{}'::jsonb); END IF;

  RAISE NOTICE 'ANCPP: % sections', (SELECT COUNT(*) FROM page_sections WHERE page_id = v_pid);
END $$;
